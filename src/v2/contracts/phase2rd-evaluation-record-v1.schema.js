...
} from "../phase2rd/phase2rd-run-contract.js";

export const PHASE2RD_EVALUATION_RECORD_VERSION =
  "phase2rd-smoke-evaluation-record-v1";
export const PHASE2RD_FROZEN_PROMPT_HASH = hashUtf8(
  NOTIFICATION_ANALYSIS_CORE_PROMPT_P2_V2,
);
export const PHASE2RD_APPROVED_AUTOMATIC_DIMENSION_CHECKS =
  PHASE2RD_CASE_IDS.length * PHASE2_AUTOMATIC_DIMENSION_NAMES.length;
export const PHASE2RD_COST_REASON =
  "The approved spend boundary is six requests with at most 8,000 output tokens and 12,000 request bytes per case; no immutable DeepSeek price snapshot was approved, so no currency estimate is invented.";

export const PHASE2RD_HISTORICAL_BASELINE = Object.freeze({
  baseline_id: "phase2b-selected-case-baseline-v1",
  source_phase: "phase2b",
  selected_case_ids: Object.freeze([...PHASE2RD_CASE_IDS]),
  valid_candidate_count: 4,
  technical_invalid_case_count: 2,
  automatic_passed_case_count: 0,
});

export const PHASE2RD_EVALUATION_CLAIMS = Object.freeze({
  can_prove: Object.freeze([
    "The six frozen synthetic Phase 2R-D captures were hash-verified before development expected data was loaded.",
    "A pending_manual_review status means six of six Candidates were technically valid and all 36 automatic dimensions were exact; it is not a full-batch approval.",
  ]),
  cannot_prove: Object.freeze([
    "This six-case visible synthetic smoke cannot prove full revised-development quality, locked-set performance, real-email quality, production readiness, or product Harness behavior.",
    "Thirty pending manual semantic reviews block any passing conclusion and cannot be treated as completed by this automatic evaluator.",
  ]),
});
...
function fail(message) {
  ...
  error.name = "Phase2rdEvaluationRecordValidationError";
  error.code = "phase2rd_evaluation_record_invalid";
  throw error;
}
...
function assertAttempt(attempt, requestPayloadHash) {
  ...
  if (
    ...
    attempt.prompt_hash !== PHASE2RD_FROZEN_PROMPT_HASH ||
    ...
    fail("A Phase 2R-D attempt record is invalid or contains unapproved fields");
  }
  if (
    ...
    fail("A Phase 2R-D attempt diagnostic is internally inconsistent");
  }
}
...
function assertTechnicalValidation(validation, valid) {
  ...
  if (
    ...
    fail("Phase 2R-D technical validation flags are invalid");
  }
}
...
function assertAutomatic(automatic) {
  if (
    ...
    fail("Phase 2R-D automatic evaluation shape is invalid");
  }
  let exact = 0;
  let tp = 0;
  let fp = 0;
  ...
  for (const name of PHASE2_AUTOMATIC_DIMENSION_NAMES) {
    ...
    if (
      ...
      fail("A Phase 2R-D automatic dimension is invalid");
    }
  ...
  if (
    ...
    fail("Phase 2R-D automatic totals do not close over their dimensions");
  }
}
...
function assertEvaluationErrors(errors) {
  if (
    ...
    fail("Phase 2R-D evaluation errors are invalid");
  }
}
...
function assertReviewQueue(reviewQueue) {
  if (
    ...
    fail("Phase 2R-D review queue must contain five pending items per case");
  }
  const codes = [];
  for (const review of reviewQueue) {
    if (
      ...
      fail("Phase 2R-D review queue must remain pending");
    }
    codes.push(review.code);
  }
  if (!sameJson([...codes].sort(), [...PHASE2_REVIEW_CODES].sort())) {
    fail("Phase 2R-D review queue is incomplete");
  }
}

function assertSemanticGate(semanticGate, valid) {
  if (
    !exactKeys(semanticGate, ["status", "issues"]) ||
    !["pass", "fail", "not_run"].includes(semanticGate.status) ||
    !Array.isArray(semanticGate.issues) ||
    semanticGate.issues.length > 64 ||
    semanticGate.issues.some(
      (issue) =>
        !exactKeys(issue, ["code", "path"]) ||
        !SNAKE_CASE_PATTERN.test(issue.code ?? "") ||
        !boundedString(issue.path, 300),
    ) ||
    (valid && semanticGate.status === "not_run") ||
    (!valid && semanticGate.status !== "not_run") ||
    (semanticGate.status === "pass" && semanticGate.issues.length !== 0) ||
    (semanticGate.status === "not_run" && semanticGate.issues.length !== 0) ||
    (semanticGate.status === "fail" && semanticGate.issues.length === 0)
  ) {
    fail("Phase 2R-D semantic gate result is invalid");
  }
}
...
function assertExcludedFields(excludedFields) {
  if (
    ...
    fail("Phase 2R-D excluded fields are invalid");
  }
}

function assertCandidateShape(shape) {
  if (!shape || typeof shape !== "object" || Array.isArray(shape)) {
    fail("Phase 2R-D failure diagnostic candidate shape is invalid");
  }
  if (shape.root_type === "object") {
    ...
    if (
      ...
      fail("Phase 2R-D object diagnostic shape is invalid");
    }
    return;
  }
  if (
    ...
    fail("Phase 2R-D scalar diagnostic shape is invalid");
  }
}
...
export function assertValidPhase2rdDiagnostic(diagnostic) {
  if (
    ...
    fail("Phase 2R-D failure diagnostic is invalid");
  }
...
function assertCase(item) {
  ...
  if (
    !exactKeys(item, keys) ||
    !PHASE2RD_CASE_IDS.includes(item.case_id) ||
    ...
    fail("Phase 2R-D case envelope is invalid");
  }
  ...
  assertSemanticGate(item.semantic_gate, valid);
  ...
  if (valid) {
    if (
      ...
      fail("A valid Phase 2R-D Candidate requires a closed successful attempt");
    }
    ...
    if (
      ...
      fail("An invalid Phase 2R-D capture is not closed safely");
    }
    assertValidPhase2rdDiagnostic(item.diagnostic);
  }
}

export function phase2rdEvaluationPayload(record) {
  const { canonical_evaluation_hash: ignored, ...payload } = record;
  void ignored;
  return payload;
}

export function computePhase2rdEvaluationHash(record) {
  return hashCanonicalJson(phase2rdEvaluationPayload(record));
}
...
export function buildPhase2rdEvaluationSummary(cases) {
  ...
  const semanticGatePassedCount = cases.filter(
    (item) => item.semantic_gate.status === "pass",
  ).length;
  const semanticGateFailedCount = cases.filter(
    (item) => item.semantic_gate.status === "fail",
  ).length;
  return {
    planned_case_count: PHASE2RD_CASE_IDS.length,
    provider_request_count: cases.length,
    valid_candidate_count: valid.length,
    technical_invalid_case_count: PHASE2RD_CASE_IDS.length - valid.length,
    semantic_gate_passed_case_count: semanticGatePassedCount,
    semantic_gate_failed_case_count: semanticGateFailedCount,
    semantic_gate_not_run_case_count:
      PHASE2RD_CASE_IDS.length - semanticGatePassedCount - semanticGateFailedCount,
    ...
    approved_automatic_dimension_check_count:
      PHASE2RD_APPROVED_AUTOMATIC_DIMENSION_CHECKS,
    ...
    approved_max_provider_requests: PHASE2RD_MAX_REQUESTS,
    approved_max_output_tokens_per_request: PHASE2RD_MAX_OUTPUT_TOKENS,
    approved_max_total_output_tokens: PHASE2RD_MAX_TOTAL_OUTPUT_TOKENS,
    approved_max_request_utf8_bytes_per_case:
      PHASE2RD_MAX_REQUEST_UTF8_BYTES,
    estimated_cost: null,
    estimated_cost_reason: PHASE2RD_COST_REASON,
  };
}

export function phase2rdPassedAutomaticGates(summary) {
  return (
    summary.valid_candidate_count === PHASE2RD_CASE_IDS.length &&
    summary.technical_invalid_case_count === 0 &&
    summary.semantic_gate_passed_case_count === PHASE2RD_CASE_IDS.length &&
    summary.semantic_gate_failed_case_count === 0 &&
    summary.semantic_gate_not_run_case_count === 0 &&
    summary.automatic_passed_case_count === PHASE2RD_CASE_IDS.length &&
...
export function assertValidPhase2rdEvaluationRecord(record) {
  if (
    !exactKeys(record, [
      "record_version",
      "run_id",
    ...
    record.record_version !== PHASE2RD_EVALUATION_RECORD_VERSION ||
    record.phase !== "phase2r-d" ||
    !["pending_manual_review", "failed"].includes(record.status) ||
    ...
    record.provider !== PHASE2RD_PROVIDER ||
    record.model !== PHASE2RD_MODEL ||
    !COMMIT_PATTERN.test(record.implementation_commit_sha ?? "") ||
    record.prompt_version !== PHASE2RD_PROMPT_VERSION ||
    record.prompt_hash !== PHASE2RD_FROZEN_PROMPT_HASH ||
    record.candidate_schema_version !== PHASE2RD_CANDIDATE_SCHEMA_VERSION ||
    record.candidate_schema_version !== PHASE2_CANDIDATE_SCHEMA_VERSION ||
    record.candidate_schema_hash !== PHASE2_CANDIDATE_SCHEMA_HASH ||
    record.diagnostic_version !== PHASE2RD_DIAGNOSTIC_VERSION ||
    record.case_set_hash !== PHASE2RD_CASE_SET_HASH ||
    record.model_input_set_hash !== PHASE2RD_MODEL_INPUT_SET_HASH ||
    record.source_context_snapshot_hash !==
      PHASE2RD_SOURCE_CONTEXT_SNAPSHOT_HASH ||
    record.source_context_file_hash !== PHASE2RD_SOURCE_CONTEXT_FILE_HASH ||
    !HASH_PATTERN.test(record.authorization_marker_hash ?? "") ||
    !HASH_PATTERN.test(record.capture_index_hash ?? "") ||
    !sameJson(record.historical_baseline, PHASE2RD_HISTORICAL_BASELINE) ||
    !Array.isArray(record.cases) ||
    record.cases.length !== PHASE2RD_CASE_IDS.length ||
    record.cases.some(
      (item, index) => item?.case_id !== PHASE2RD_CASE_IDS[index],
    ) ||
    !sameJson(record.claims, PHASE2RD_EVALUATION_CLAIMS)
  ) {
    fail("Phase 2R-D evaluation record envelope is invalid");
  }
  ...
  if (
    ...
    fail("Phase 2R-D evaluation safety assertion is invalid");
  }

  for (const item of record.cases) assertCase(item);
  const summary = buildPhase2rdEvaluationSummary(record.cases);
  if (!sameJson(record.summary, summary)) {
    fail("Phase 2R-D summary does not close over the six case records");
  }
  if (
    record.summary.pending_manual_review_count !== 30 ||
    record.summary.full_batch_execution_blocked_by_pending_manual_review !== true
  ) {
    fail("Phase 2R-D must retain all 30 pending manual reviews");
  }
  const expectedStatus = phase2rdPassedAutomaticGates(summary)
    ? "pending_manual_review"
    : "failed";
  if (record.status !== expectedStatus) {
    fail("Phase 2R-D status does not match its strict smoke gate");
  }
  if (
    record.canonical_evaluation_hash !==
      computePhase2rdEvaluationHash(record)
  ) {
    fail("Phase 2R-D canonical evaluation hash is invalid");
  }
  if (Buffer.byteLength(canonicalJsonStringify(record), "utf8") > 750_000) {
    fail("Phase 2R-D evaluation record is too large");
  }
  return record;
}
