...
import {
  PHASE2RB_EVALUATION_CLAIMS,
  PHASE2RB_EVALUATION_RECORD_VERSION,
  PHASE2RB_FROZEN_PROMPT_HASH,
  PHASE2RB_HISTORICAL_BASELINE,
  assertValidPhase2rbEvaluationRecord,
  buildPhase2rbEvaluationSummary,
  computePhase2rbEvaluationHash,
  phase2rbPassedForFullBatchDecision,
} from "../contracts/phase2rb-evaluation-record-v1.schema.js";
import {
...
export class Phase2rbEvaluationError extends Error {
  constructor(code, message, options = {}) {
    super(message, options);
    this.name = "Phase2rbEvaluationError";
    this.code = code;
  }
}
...
function fail(code, message, options) {
  throw new Phase2rbEvaluationError(code, message, options);
}
...
function expectedRequestDescriptor(modelInput) {
  ...
  if (requestUtf8Bytes > PHASE2RB_MAX_REQUEST_UTF8_BYTES) {
    fail(
      "phase2rb_capture_invalid",
      "A frozen Phase 2R-B request exceeds its approved byte budget.",
    );
  }
  return {
...
function assertCaptureEnvelope(marker, index) {
  try {
    ...
    fail(
      "phase2rb_capture_invalid",
      "The Phase 2R-B marker or capture index failed its exact contract.",
      { cause: error },
    );
  }
  if (
    ...
    fail(
      "phase2rb_capture_invalid",
      "The Phase 2R-B marker or capture index is incomplete or inconsistent.",
    );
  }
}
...
function selectAndVerifyInputs(allInputs) {
  if (
    ...
    fail(
      "phase2rb_input_invalid",
      "The frozen answer-free Phase 2R Input set drifted.",
    );
  }
  return PHASE2RB_CASE_IDS.map(
...
async function loadAndVerifyCaptures({
  ...
  const runDirectory = phase2rbRunDirectory(marker.run_id, { runtimeDirectory });
  const verified = [];
  for (let caseIndex = 0; caseIndex < PHASE2RB_CASE_IDS.length; caseIndex += 1) {
    const caseId = PHASE2RB_CASE_IDS[caseIndex];
    ...
    try {
      ...
      fail(
        "phase2rb_capture_invalid",
        "A Phase 2R-B intent or terminal failed its exact contract.",
        { cause: error },
      );
    }
    if (
      ...
      fail(
        "phase2rb_capture_invalid",
        "A Phase 2R-B intent or terminal failed full hash verification.",
      );
    }
    let candidate = null;
    if (terminal.status === "candidate_valid") {
      ...
      try {
        ...
        fail(
          ...
          "A captured Phase 2R-B Candidate failed final pre-truth validation.",
          { cause: error },
        );
      }
    } else {
      if (
        ...
        fail(
          "phase2rb_capture_invalid",
          "An invalid Phase 2R-B capture retained unsafe or incomplete data.",
        );
      }
    }
...
function verifyTruthCase(developmentCase, truthEntry, caseId) {
  if (
    ...
    fail(
      ...
      "A frozen Phase 2R-B development expected record drifted.",
    );
  }
  ...
  if (hashCanonicalJson(oracle) !== truthEntry.oracleHash) {
    fail(
      "phase2_evaluation_truth_invalid",
      "A frozen Phase 2R-B Oracle hash drifted.",
    );
  }
  return oracle;
}
...
function recordCase({ input, capture, oracle }) {
  ...
  if (hashCanonicalJson(candidate) !== before) {
    fail(
      "phase2rb_candidate_integrity_failed",
      "Evaluation changed a captured Phase 2R-B Candidate.",
    );
  }
  return {
    case_id: input.caseId,
...
export async function runPhase2rbEvaluation({
  ...
  try {
    ...
    fail(
      "phase2rb_capture_invalid",
      "The Phase 2R-B authorization marker failed its exact contract.",
      { cause: error },
    );
  }
  const runDirectory = phase2rbRunDirectory(marker.run_id, { runtimeDirectory });
  ...
  if (
    ...
    fail(
      "phase2_evaluation_truth_invalid",
      "The frozen Phase 2R-B development truth set is incomplete.",
    );
  }
  const selectedTruth = PHASE2RB_CASE_IDS.map((caseId) => {
  ...
  const cases = PHASE2RB_CASE_IDS.map((_, selectedIndex) =>
    recordCase({
  ...
  const summary = buildPhase2rbEvaluationSummary(cases);
  const record = {
    record_version: PHASE2RB_EVALUATION_RECORD_VERSION,
    run_id: marker.run_id,
    phase: "phase2r-b",
    status: phase2rbPassedForFullBatchDecision(summary)
    ...
    prompt_hash: PHASE2RB_FROZEN_PROMPT_HASH,
    ...
    historical_baseline: {
      ...PHASE2RB_HISTORICAL_BASELINE,
      selected_case_ids: [...PHASE2RB_HISTORICAL_BASELINE.selected_case_ids],
    },
    cases,
    summary,
    claims: {
      can_prove: [...PHASE2RB_EVALUATION_CLAIMS.can_prove],
      cannot_prove: [...PHASE2RB_EVALUATION_CLAIMS.cannot_prove],
    },
    canonical_evaluation_hash: "",
  };
  record.canonical_evaluation_hash = computePhase2rbEvaluationHash(record);
  assertValidPhase2rbEvaluationRecord(record);
  const written = await writeRecordImpl(record, { runtimeDirectory });
  return Object.freeze({
    exitCode: record.status === "passed_for_full_batch_decision" ? 0 : 5,
    runId: record.run_id,
