...
export class Phase2rbCaptureContractError extends Error {
  constructor(message) {
    super(message);
    this.name = "Phase2rbCaptureContractError";
    this.code = "phase2rb_capture_contract_invalid";
  }
}

function fail(message) {
  throw new Phase2rbCaptureContractError(message);
}
...
function assertCase(caseId, caseIndex) {
  if (
    ...
    caseIndex < 0 ||
    caseIndex >= PHASE2RB_CASE_IDS.length ||
    caseId !== PHASE2RB_CASE_IDS[caseIndex]
  ) {
    fail("Case identity does not match the frozen Phase 2R-B order");
  }
}
...
function orderedSubset(values) {
  ...
  for (const value of values) {
    const index = PHASE2RB_CASE_IDS.indexOf(value);
...
export function assertValidPhase2rbAttempt(attempt, requestPayloadHash) {
  if (
    ...
    attempt.prompt_hash !== PHASE2RB_PROMPT_HASH ||
...
function assertDescriptor(descriptor, caseIndex) {
  ...
  if (
    ...
    descriptor.prompt_hash !== PHASE2RB_PROMPT_HASH ||
    descriptor.schema_hash !== PHASE2RB_SCHEMA_HASH ||
...
export function assertValidPhase2rbAuthorizationMarker(marker) {
  if (
    !exactKeys(marker, MARKER_KEYS) ||
    marker.authorization_version !== PHASE2RB_AUTHORIZATION_VERSION ||
    marker.authorization_id !== PHASE2RB_AUTHORIZATION_ID ||
    ...
    !sameJson(marker.case_ids, PHASE2RB_CASE_IDS) ||
    marker.case_set_hash !== PHASE2RB_CASE_SET_HASH ||
    marker.provider !== PHASE2RB_PROVIDER ||
    marker.model !== PHASE2RB_MODEL ||
    marker.prompt_version !== PHASE2RB_PROMPT_VERSION ||
    marker.prompt_hash !== PHASE2RB_PROMPT_HASH ||
    marker.candidate_schema_version !== PHASE2RB_CANDIDATE_SCHEMA_VERSION ||
    marker.schema_hash !== PHASE2RB_SCHEMA_HASH ||
    marker.diagnostic_version !== PHASE2RB_DIAGNOSTIC_VERSION ||
    marker.base_snapshot_hash !== PHASE2RB_BASE_SNAPSHOT_HASH ||
    marker.base_snapshot_file_hash !== PHASE2RB_BASE_SNAPSHOT_FILE_HASH ||
    marker.model_input_set_hash !== PHASE2RB_MODEL_INPUT_SET_HASH ||
    marker.source_context_snapshot_hash !==
      PHASE2RB_SOURCE_CONTEXT_SNAPSHOT_HASH ||
    marker.source_context_file_hash !== PHASE2RB_SOURCE_CONTEXT_FILE_HASH ||
    !Array.isArray(marker.request_descriptors) ||
    marker.request_descriptors.length !== PHASE2RB_MAX_REQUESTS ||
    marker.request_descriptor_set_hash !==
      hashCanonicalJson(marker.request_descriptors) ||
    marker.max_requests !== PHASE2RB_MAX_REQUESTS ||
    marker.requests_per_case !== PHASE2RB_REQUESTS_PER_CASE ||
    marker.serial !== PHASE2RB_SERIAL ||
    marker.retries !== PHASE2RB_RETRIES ||
    marker.max_output_tokens !== PHASE2RB_MAX_OUTPUT_TOKENS ||
    marker.timeout_ms !== PHASE2RB_TIMEOUT_MS ||
    marker.data_scope !== PHASE2RB_DATA_SCOPE
  ) {
...
export function assertValidPhase2rbRequestIntent(intent) {
  assertCase(intent?.case_id, intent?.case_index);
  if (
    !exactKeys(intent, INTENT_KEYS) ||
    intent.capture_file_version !== PHASE2RB_CAPTURE_FILE_VERSION ||
    ...
    intent.provider !== PHASE2RB_PROVIDER ||
    intent.model !== PHASE2RB_MODEL ||
    intent.prompt_version !== PHASE2RB_PROMPT_VERSION ||
    !HASH_PATTERN.test(intent.model_input_hash ?? "") ||
    intent.prompt_hash !== PHASE2RB_PROMPT_HASH ||
    intent.schema_hash !== PHASE2RB_SCHEMA_HASH ||
    !HASH_PATTERN.test(intent.request_payload_hash ?? "") ||
    !Number.isSafeInteger(intent.request_utf8_bytes) ||
    intent.request_utf8_bytes < 1 ||
    intent.request_utf8_bytes > PHASE2RB_MAX_REQUEST_UTF8_BYTES ||
    intent.max_output_tokens !== PHASE2RB_MAX_OUTPUT_TOKENS ||
    intent.timeout_ms !== PHASE2RB_TIMEOUT_MS
  ) {
...
export function assertValidPhase2rbCaseTerminal(terminal) {
  assertCase(terminal?.case_id, terminal?.case_index);
  if (
    !exactKeys(terminal, TERMINAL_KEYS) ||
    terminal.capture_file_version !== PHASE2RB_CAPTURE_FILE_VERSION ||
  ...
  if (terminal.status === "candidate_valid") {
    ...
    assertValidPhase2rbDiagnostic(terminal.diagnostic);
  }
  return terminal;
}

export function assertValidPhase2rbCaptureIndex(index) {
  if (
    !exactKeys(index, INDEX_KEYS) ||
    index.capture_file_version !== PHASE2RB_CAPTURE_FILE_VERSION ||
    ...
    index.provider !== PHASE2RB_PROVIDER ||
    index.model !== PHASE2RB_MODEL ||
    index.prompt_version !== PHASE2RB_PROMPT_VERSION ||
    index.planned_case_count !== PHASE2RB_MAX_REQUESTS ||
    index.request_intent_count !== PHASE2RB_MAX_REQUESTS ||
    index.provider_request_count !== PHASE2RB_MAX_REQUESTS ||
    index.terminal_count !== PHASE2RB_MAX_REQUESTS ||
    !Array.isArray(index.terminals) ||
    index.terminals.length !== PHASE2RB_MAX_REQUESTS ||
    index.terminals.some(
      (reference, caseIndex) =>
        !exactKeys(reference, ["case_id", "case_index", "terminal_hash"]) ||
        reference.case_id !== PHASE2RB_CASE_IDS[caseIndex] ||
...
export function assertValidPhase2rbBatchTerminal(terminal) {
  if (
    ...
    terminal.planned_case_count !== PHASE2RB_MAX_REQUESTS ||
    ...
    terminal.request_intent_count > PHASE2RB_MAX_REQUESTS ||
    terminal.provider_request_count > 16 ||
    terminal.case_terminal_count > PHASE2RB_MAX_REQUESTS ||
...
export function assertValidPhase2rbCaptureFile(value) {
  if (value?.kind === "request_intent") {
    return assertValidPhase2rbRequestIntent(value);
  }
  if (value?.kind === "case_terminal") {
    return assertValidPhase2rbCaseTerminal(value);
  }
  if (value?.kind === "capture_index") {
    return assertValidPhase2rbCaptureIndex(value);
  }
  if (value?.kind === "batch_terminal") {
    return assertValidPhase2rbBatchTerminal(value);
  }
  fail("Unknown Phase 2R-B capture file kind");
}
