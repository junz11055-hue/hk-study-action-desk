...
} from "../model/phase2r-core-model-adapter.js";
...
import { assertValidPhase2rbAuthorizationMarker } from "./phase2rb-capture-contract.js";
import {
  PHASE2RB_BASE_URL,
...
import {
  ...
  writePhase2rbCaptureIndex,
  writePhase2rbCaseTerminal,
  writePhase2rbRequestIntent,
...
export class Phase2rbCaptureError extends Error {
  constructor(code, message, options = {}) {
    super(message, options);
    this.name = "Phase2rbCaptureError";
    this.code = code;
...
export class Phase2rbRequestBudget {
  #caseIds;
  #used = [];

  constructor(caseIds = PHASE2RB_CASE_IDS) {
    if (
      !Array.isArray(caseIds) ||
      canonicalJsonStringify(caseIds) !==
        canonicalJsonStringify(PHASE2RB_CASE_IDS)
    ) {
      throw new TypeError("Phase 2R-B budget requires the four frozen cases");
    }
  ...
  reserve(caseId) {
    ...
    if (expected === undefined) {
      throw new Phase2rbCaptureError(
        "phase2rb_request_budget_exhausted",
        "The Phase 2R-B batch cannot exceed four request intents.",
      );
    }
    if (caseId !== expected || this.#used.includes(caseId)) {
      throw new Phase2rbCaptureError(
        "phase2rb_case_order_invalid",
        "Phase 2R-B cases must be requested once in frozen order.",
      );
    }
...
function safeErrorCode(value, fallback = "phase2rb_capture_failed") {
...
function assertAuthorizationMarker(
  ...
  try {
    assertValidPhase2rbAuthorizationMarker(marker);
  } catch {
    throw new Phase2rbCaptureError(
      "phase2rb_authorization_marker_invalid",
      "The durable Phase 2R-B marker does not match the frozen batch.",
    );
  }
  if (
    marker.run_id !== runId ||
    ...
    throw new Phase2rbCaptureError(
      "phase2rb_authorization_marker_invalid",
      "The durable Phase 2R-B marker does not match this frozen run.",
    );
  }
}
...
function assertClient(modelClient) {
  if (
    modelClient?.provider !== PHASE2RB_PROVIDER ||
    modelClient?.configured !== true ||
    modelClient?.model !== PHASE2RB_MODEL ||
    modelClient?.baseUrl !== PHASE2RB_BASE_URL ||
    modelClient?.timeoutMs !== PHASE2RB_TIMEOUT_MS ||
    modelClient?.maxRetries !== PHASE2RB_CLIENT_MAX_RETRIES ||
    modelClient?.logger != null ||
    typeof modelClient?.createStructuredAttempt !== "function"
  ) {
    throw new Phase2rbCaptureError(
      "model_configuration_invalid",
      "Phase 2R-B DeepSeek configuration is not frozen.",
    );
  }
}
...
function selectInputs(inputs) {
  if (
    ...
    throw new Phase2rbCaptureError(
      "phase2rb_input_set_invalid",
      "The answer-free Phase 2R Input set is not frozen.",
    );
  }
  const byId = new Map(inputs.map((input) => [input.caseId, input]));
  const selected = PHASE2RB_CASE_IDS.map((caseId) => byId.get(caseId));
  if (selected.some((input, index) => input?.caseId !== PHASE2RB_CASE_IDS[index])) {
    throw new Phase2rbCaptureError(
      "phase2rb_input_set_invalid",
      "A frozen Phase 2R-B case is unavailable.",
    );
  }
  return selected;
}
  ...
  return Object.freeze(
    inputs.map((input, caseIndex) => {
      ...
      if (
        ...
        descriptor.prompt_version !== PHASE2RB_PROMPT_VERSION ||
        descriptor.request_utf8_bytes > PHASE2RB_MAX_REQUEST_UTF8_BYTES
      ) {
        throw new Phase2rbCaptureError(
          "phase2rb_input_set_invalid",
          "A frozen Phase 2R-B request descriptor drifted.",
        );
      }
      return Object.freeze({
...
export async function loadPhase2rbRequestDescriptors({
  readFileImpl,
...
function rebuildValidation(validation, requireAllTrue) {
  if (
    ...
    throw new Phase2rbCaptureError(
      "phase2rb_validation_invalid",
      "A Phase 2R-B validation result crossed its frozen contract.",
    );
  }
...
function rebuildCandidateShape(shape) {
  if (!shape || typeof shape !== "object" || Array.isArray(shape)) {
    throw new Phase2rbCaptureError(
      "phase2rb_diagnostic_invalid",
      "A Phase 2R-B diagnostic shape is invalid.",
    );
  }
  if (shape.root_type !== "object") {
    if (!exactKeys(shape, ["root_type"]) || !ROOT_TYPES.has(shape.root_type)) {
      throw new Phase2rbCaptureError(
        "phase2rb_diagnostic_invalid",
        "A Phase 2R-B diagnostic root type is invalid.",
      );
    }
  ...
  if (
    ...
    throw new Phase2rbCaptureError(
      "phase2rb_diagnostic_invalid",
      "A Phase 2R-B diagnostic count is invalid.",
    );
  }
  return {
    root_type: "object",
...
function rebuildDiagnostic(diagnostic) {
  if (
    ...
    throw new Phase2rbCaptureError(
      "phase2rb_diagnostic_invalid",
      "A Phase 2R-B diagnostic crossed its whitelist.",
    );
  }
  return {
...
function rebuildAttempt(attempt, descriptor) {
  if (
    ...
    throw new Phase2rbCaptureError(
      "phase2rb_attempt_integrity_failed",
      "A Phase 2R-B attempt crossed its frozen contract.",
    );
  }
  return {
    attempt: 1,
...
export function createPhase2rbFailedBatchTerminal({
  runId,
  ...
  const unattempted = PHASE2RB_CASE_IDS.filter(
    (caseId) => !attemptedSet.has(caseId),
  );
  return {
    capture_file_version: PHASE2RB_CAPTURE_FILE_VERSION,
    kind: "batch_terminal",
    run_id: runId,
    status: "failed",
    finished_at: isoNow(clock),
    implementation_commit_sha: implementationCommitSha,
    planned_case_count: PHASE2RB_MAX_REQUESTS,
...
/**
 * Capture four fixed synthetic cases serially. This module intentionally has
 * no expected fixture, Oracle, evaluator, product service, or listener import.
 */
export async function capturePhase2rbCandidates({
  runId,
  implementationCommitSha,
  modelClient,
  runtimeDirectory,
  readFileImpl,
  readMarkerImpl = readPhase2rbAuthorizationMarker,
  loadInputsImpl = loadPhase2rDevelopmentInputs,
  analyzeImpl = analyzePhase2rCoreCandidate,
  writeIntentImpl = writePhase2rbRequestIntent,
  writeTerminalImpl = writePhase2rbCaseTerminal,
  writeIndexImpl = writePhase2rbCaptureIndex,
  writeBatchTerminalImpl = writePhase2rbBatchTerminal,
  beforeCasePreflight,
  budget = new Phase2rbRequestBudget(),
  clock = () => new Date(),
  onProgress,
} = {}) {
  ...
  try {
    if (!GIT_COMMIT_PATTERN.test(implementationCommitSha ?? "")) {
      throw new Phase2rbCaptureError(
        ...
        "Phase 2R-B requires a frozen clean implementation commit.",
      );
    }
    ...
    if (typeof beforeCasePreflight !== "function") {
      throw new Phase2rbCaptureError(
        "implementation_not_frozen",
        "Phase 2R-B requires a per-case clean Git preflight.",
      );
    }
    ...
    for (let caseIndex = 0; caseIndex < inputs.length; caseIndex += 1) {
      ...
      if (ordinal !== caseIndex + 1) {
        throw new Phase2rbCaptureError(
          "phase2rb_case_order_invalid",
          "Phase 2R-B request reservation order drifted.",
        );
      }
      const descriptor = buildPhase2rRequestDescriptor(input.modelInput);
      if (
      ...
      try {
        ...
        throw new Phase2rbCaptureError(
          "implementation_not_frozen",
          "The Phase 2R-B implementation drifted before a request intent.",
        );
      }
      if (
      ...
      if (!Array.isArray(rawAttempts)) {
        throw new Phase2rbCaptureError(
          "phase2rb_attempt_integrity_failed",
          "A Phase 2R-B attempt list is invalid.",
        );
      }
      ...
      if (rawAttempts.length > 1) {
        throw new Phase2rbCaptureError(
          "phase2rb_request_budget_exhausted",
          "A Phase 2R-B case attempted more than one provider request.",
        );
      }
      const attempt =
      ...
      if (analysis !== null) {
        ...
        if (candidateHash !== analysis.candidateHash) {
          throw new Phase2rbCaptureError(
            "phase2rb_candidate_integrity_failed",
            "A Phase 2R-B Candidate changed before terminal capture.",
          );
        }
      } else {
        if (
          ...
          throw new Phase2rbCaptureError(
            "phase2rb_candidate_integrity_failed",
            "A Phase 2R-B Candidate hash is invalid.",
          );
        }
        diagnostic =
      ...
      if (error && SYSTEMIC_STOP_CODES.has(error.code)) {
        throw new Phase2rbCaptureError(
          "phase2rb_systemic_request_failure",
          "Phase 2R-B stopped after a systemic request failure.",
          { triggerCode: error.code },
        );
      }
    }

    if (!budget.complete || terminalRefs.length !== PHASE2RB_MAX_REQUESTS) {
      throw new Phase2rbCaptureError(
        "phase2rb_capture_incomplete",
        "Phase 2R-B did not persist all four case terminals.",
      );
    }
    const captureIndex = {
      capture_file_version: PHASE2RB_CAPTURE_FILE_VERSION,
    ...
    const controlled =
      error instanceof Phase2rbCaptureError
        ? error
        : new Phase2rbCaptureError(
            safeErrorCode(error?.code),
            "The Phase 2R-B capture stopped safely.",
            { cause: error },
          );
    const failedTerminal = createPhase2rbFailedBatchTerminal({
      runId,
      implementationCommitSha,
    ...
    try {
      ...
      throw new Phase2rbCaptureError(
        "phase2rb_batch_terminal_write_failed",
        "The failed Phase 2R-B batch could not persist its terminal record.",
        {
          cause: writeError,
