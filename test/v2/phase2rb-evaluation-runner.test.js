...
} from "../../src/v2/contracts/phase2rb-evaluation-record-v1.schema.js";
import {
...
import {
  runPhase2rbEvaluation,
} from "../../src/v2/phase2rb/phase2rb-evaluation-runner.js";
import {
...
async function evaluationFixture({ mutateCandidate } = {}) {
  ...
  const selectedInputs = PHASE2RB_CASE_IDS.map((caseId) =>
...
function optionsFor(fixture, overrides = {}) {
  return {
    runtimeDirectory: "/synthetic/phase2rb-runtime",
    ...
    writeRecordImpl: async (record) => ({
      path: "/synthetic/phase2rb-runtime/evaluation.json",
...
test("Phase 2R-B pass requires 4/4 valid Candidates and 24/24 exact dimensions", async () => {
  const fixture = await evaluationFixture();
  const result = await runPhase2rbEvaluation(optionsFor(fixture));

  assert.equal(result.exitCode, 0);
  assert.equal(result.record.status, "passed_for_full_batch_decision");
  assert.equal(result.record.summary.valid_candidate_count, 4);
  assert.equal(result.record.summary.technical_invalid_case_count, 0);
  assert.equal(result.record.summary.automatic_passed_case_count, 4);
  assert.equal(result.record.summary.automatic_dimension_check_count, 24);
  assert.equal(result.record.summary.automatic_dimension_exact_count, 24);
  assert.equal(result.record.summary.p0_error_count, 0);
  assert.equal(result.record.summary.p1_error_count, 0);
  assert.equal(result.record.summary.pending_manual_review_count, 20);
  assert.equal(
    result.record.summary.full_batch_execution_blocked_by_pending_manual_review,
    true,
  );
  assertValidPhase2rbEvaluationRecord(result.record);
...
test("Expected is loaded only after the complete capture chain is verified", async () => {
  ...
  let captureReads = 0;
  let truthLoads = 0;
  const result = await runPhase2rbEvaluation(
    optionsFor(fixture, {
      readCaptureImpl: async (filePath) => {
        captureReads += 1;
        const value = fixture.captures.get(path.basename(filePath));
...
test("An incomplete or hash-drifted capture blocks truth loading and evaluation", async () => {
  ...
  await assert.rejects(
    ...
    (error) => error.code === "phase2rb_capture_invalid",
  );
  assert.equal(truthLoads, 0);
});

test("Any automatic semantic miss fails the four-case smoke gate", async () => {
  const fixture = await evaluationFixture({
  ...
  assert.equal(result.record.summary.valid_candidate_count, 4);
  assert.ok(result.record.summary.automatic_dimension_exact_count < 24);
  assert.ok(
