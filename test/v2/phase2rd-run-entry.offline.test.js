...
import { main } from "../../src/v2/phase2rd/run-phase2rd-deepseek.js";
import {
  PHASE2RD_CASE_IDS,
...
const KEY_CANARY = "phase2rd-entry-key-canary-never-print";
const ERROR_CANARY = "phase2rd-entry-error-canary-never-print";
...
async function tempRuntime(t) {
  const created = await mkdtemp(path.join(os.tmpdir(), "phase2rd-entry-"));
...
test("invalid Phase 2R-D CLI performs zero Git, descriptor, marker, Key, client, or capture work", async () => {
  const calls = {
    inspect: 0,
    descriptors: 0,
    marker: 0,
...
test("runner consumes the marker before Key access and keeps six fake cases serial", async () => {
  const order = [];
  const stdout = sink();
  const stderr = sink();
  let inspection = 0;
  const result = await main([], {
    ...
    loadRequestDescriptorsImpl: async () => {
      ...
      return await loadPhase2rdRequestDescriptors();
    },
    createAuthorizationMarkerImpl: async (marker) => {
      order.push("marker");
      assert.equal(marker.status, "consumed");
      assert.deepEqual(marker.case_ids, PHASE2RD_CASE_IDS);
    ...
    captureImpl: async ({ beforeCasePreflight, onProgress, modelClient }) => {
      ...
      for (let index = 0; index < PHASE2RD_CASE_IDS.length; index += 1) {
        ...
        onProgress({
          completed: index + 1,
          planned: PHASE2RD_CASE_IDS.length,
          case_id: caseId,
  ...
  assert.deepEqual(
    ...
    PHASE2RD_CASE_IDS.map((caseId) => `fake-attempt:${caseId}`),
  );
  assert.equal(inspection, 2 + PHASE2RD_CASE_IDS.length);
...
test("Git drift after Key access consumes authorization and performs zero capture work", async (t) => {
  ...
  assert.deepEqual(terminal.unattempted_case_ids, PHASE2RD_CASE_IDS);
